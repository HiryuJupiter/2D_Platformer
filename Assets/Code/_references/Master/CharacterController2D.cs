...
namespace Prime31
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        [Range(0f, 90f)]
        public float slopeLimit = 30f; //That allows climbing
            ...
            for (var i = 0; i < totalHorizontalRays; i++)
            {
                ...
                if (_raycastHit)
                {
                    ...
                    if (i == 0 && handleHorizontalSlope(ref deltaMovement, Vector2.Angle(_raycastHit.normal, Vector2.up)))
                    {
                        ...
                        {
                            float flushDistance = Mathf.Sign(deltaMovement.x) * (_raycastHit.distance);
        ...
        private void handleVerticalSlope(ref Vector3 deltaMovement)
        {
            ...
            var slopeCheckRayDistance = _slopeLimitTangent * (_raycastOrigins.bottomRight.x - transform.position.x);

            var slopeRay = new Vector2(transform.position.x, _raycastOrigins.bottomLeft.y);
